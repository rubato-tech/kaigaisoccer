...
## WC2026データソース変更（worldcup26.ir対応）（2026-06-30）

- [x] TheSportsDBのWC2026データが不完全（20/104試合のみ）と判明
- [x] worldcup26.ir（無料・認証不要）に切り替え
- [x] sync-direct.tsにsyncWc2026関数を実装（全104試合取得）
- [x] TheSportsDB由来の古いデータを自動削除するロジックを追加
- [x] ローカルテスト: fetched=104 upserted=104 errors=0 確認済み
- [x] DB確認: グループステージ72試合＋決勝T32試合（全9ラウンド）が正しく保存済み
- [x] チェックポイント保存・GitHubプッシュ

## 2026-27欧州日程の取得・表示確認（2026-08-21）

- [x] kaigaisoccer.comの欧州日程タブとRailway DBでプレミアリーグの8月23日JST試合を照合する
...
## TimeTree予定追加対応（2026-08-21）

- [x] TimeTreeがサポートする予定追加・カレンダー購読形式を公式情報で確認する
- [x] 既存のGoogleカレンダー・iCalendar連携を確認し、TimeTree用導線を設計する
- [x] 個別試合とお気に入りチーム予定をTimeTreeで利用できるよう実装する
- [x] モバイル表示・予定データ・Vitestテストを検証する
- [x] チェックポイント保存・利用手順を案内する

## 全取得対象の日程完全性監査（2026-08-21）

- [x] 対象リーグ・国内カップ・UEFA大会・代表戦の全一覧を確定し、本番DBの2026-27件数と期間を棚卸しする
- [x] 各大会の現行データソースと公式または信頼できる日程ソースを照合する
- [x] カラバオカップ、FAカップ、CL、EL、ECLを含む不完全な取得ロジックを修正する
- [x] 本番Railway DBに全対象の最新日程を同期する（GitHub Actions #34189598575、最新コミット9d873393で成功）
- [x] 主要試合と本番画面を横断検証する（欧州日程API 288試合・16リーグ、カラバオ杯・CL・EL表示を確認）
- [x] テスト・チェックポイント保存・反映結果を報告する

## 本番の重複試合掲載修正（2026-09-17）

- [x] kaigaisoccer.com の欧州日程APIを確認し、同一カード29組（58件）の重複を特定する
- [x] ESPN同期失敗時にTheSportsDBフォールバックが既存ESPN行と共存することを根本原因として特定する
- [x] 同一大会・キックオフ・ホーム・アウェーを基準に、ESPNを優先して重複行を安全に整理するロジックを追加する
- [x] ESPNフォールバック同期後に重複整理を実行し、再発を防止する
- [x] 回帰テスト、型チェック、ビルドを実施する（33件合格・4DB統合テストスキップ）
- [x] GitHub ActionsでRailway本番DBに同期し、本番APIの重複がゼロであることを検証する（run #35218230783、4,336件・errors=0、欧州日程API 134件・重複0組）
- [x] チェックポイントを保存し、反映結果を報告する

## 日本人選手所属クラブの2026-27更新（2026-09-17）

- [x] 2025-26シーズン基準の現行リストを監査し、2026-27の移籍後所属と照合する
- [x] ESPNの実データでクラブ名表記（例: Lille、Lyon、Feyenoord Rotterdam、Borussia Mönchengladbach）を確認する
- [x] 選手名・クラブ名・データソース別の別名を持つ所属クラブ定義へ更新する
- [x] 旧所属クラブを除外し、最新の対象クラブへ日本人選手タグを付与する
- [x] 回帰テスト、型チェック、ビルドを実施する（34件合格・外部API/DB統合6件スキップ）
- [x] GitHub Actionsで本番Railway DBを再同期し、日本人選手タブを検証する（run #35236301709、4,349件・errors=0、本番21試合・9リーグ、旧所属クラブ0件）
- [x] チェックポイント保存・移籍市場ごとの更新運用を記録する（毎年2月3日・9月3日 10:00 JSTの監査タスクを有効化）
